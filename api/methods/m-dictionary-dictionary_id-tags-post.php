...
$app->post($route, function ($Certification_ID)  use ($app){
	...
	if(isset($param['tag']))
		...
		$CheckTagPivotQuery = "SELECT * FROM dictionary_tag_pivot where Tag_ID = " . $tag_id . " AND Certification_ID = " . trim($dictionary_id);
		...
		if($CheckTagPivotResult && mysql_num_rows($CheckTagPivotResult))
			...
			{
			$query = "INSERT INTO dictionary_tag_pivot(Tag_ID,Certification_ID) VALUES(" . $tag_id . "," . $dictionary_id . "); ";
			mysql_query($query) or die('Query failed: ' . mysql_error());
			}
